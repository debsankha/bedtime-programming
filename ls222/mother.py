...
class mouse:
	def __init__(self):
		...
		self.death_probab=0				# will vary as (1-1/t)
		self.is_virgin=1
...
class felix:
	def __init__(self):
		...
		self.mature_size=normalvariate(48,12)
		self.reproduction_gap=normalvariate(2,0.6)
...
def dynamics():
	global t,starved_cat,random_dead_cat,random_dead_mouse,eaten_mouse
	mouse_no=100
	cat_no=50
	rice=500
	dt=0.2
	...
	#				print 'a mouse reproduced'
		...
		for cat in cats:
			...
			cat.size-=cat.metabolism_rate*dt						# weight loss due to weight loss
...
dynamics()
print 'tot mice\trandomly dead mice\teaten mice\ttot cats\trandomly dead cats\tstarved cats'
print len(mice),'\t\t',random_dead_mouse,'\t\t\t',eaten_mouse,'\t\t',len(cats),'\t\t',random_dead_cat,'\t\t\t',starved_cat
