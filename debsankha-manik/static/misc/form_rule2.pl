...
find_rec_pattern(P,N):-length(P,Totlen),retractall(exception(_,_)),retractall(cont(_,_)),retractall(count(_)),retractall(diff(_,_)),retractall(link_with_list_no(_,_)),asserta(count(1)),not((member(PH,P),count(Ind),retractall(count(_)),Ind1 is Ind+1,asserta(count(Ind1)),nl,write(PH),nl, not((link_with_other_lists(P,Ind,L,Num),/*reverse(L,[Num|LT0]),*/
...
((length(PH,1),used(TotLi,N),nth1(Num,TotLi,Oth),=(PH,[PHelem]),nth1(Pos,Oth,PHelem),length(Oth,Pos),last(L,tail))->(append(L,[first],LT));(=(L,LT))),
	(link_with_list_no(Num,Li)-> 
		(diff(Diff,Num)->
			...
			(assertz(exception(PH,N)),nl));
		(writef("diff not found"),nl,retractall(link_with_list_no(Num,_)),asserta(link_with_list_no(Num,LT)),append(Diff,Li,LT),\=(Diff,[]),asserta(diff(Diff,Num)),((\=(LT,[none]))->(asserta(cont(PH,N)));1=1),nl));
